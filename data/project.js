export const projects = [
  {
    title: "Bank Marketing Model",
    year: "2026",
    status: "In Progress",
    description: "Python data science project analyzing Portuguese bank telemarketing data and predicting term-deposit subscriptions using classification models.",
    technologies: ["Python", "Numpy", "Pandas", "Matplotlib","Jupyter Notebook"],
    link: "https://github.com/tuhinmohammed/bank-marketing-subscription-prediction.git",
  },
    {
    title: "Project 2",
    year: "2024",
    status: "In Progress",
    description: "Project 2 description",
    technologies: ["Next.js", "React", "JavaScript"],
    link: "#",
  },
    {
    title: "Project 3",
    year: "2024",
    status: "Live",
    description: "Project 3 description",
    technologies: ["Next.js", "React", "JavaScript"],
    link: "#",
  },
    {
    title: "Project 4",
    year: "2024",
    status: "In Progress",
    description: "Project 4 description",
    technologies: ["Next.js", "React", "JavaScript"],
    link: "#",
  }
];
